...
// You will be given an array of strings that must be parsed as numbers, representing the initial height of mile-long sections of the wall,
// in feet. Each section has its construction crew that can add 1 foot of height per day by using 195 cubic yards of concrete.
// All crews work simultaneously (see examples), meaning all sections that aren't completed (are less than 30 feet high) grow by 1 foot every day.
...
function buildWall(arr) {
  let dailyFeetConcrete = 0;
  let loopsCount = 30 - Math.min(...arr);
  let eachDayArray = [];
  ...
  for (let i = 0; i < loopsCount; i++) {
    ...
    eachDayArray.push(dailyFeetConcrete * 195);
  }
  console.log(eachDayArray.join(", "));
  console.log(`${eachDayArray.reduce((a, b) => a + b) * 1900} pesos`);
}
