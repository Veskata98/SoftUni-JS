...
function sorting(arr) {
  let descSortedArr = arr.sort((a, b) => b - a);
  let resultArr = [];

  let descIndex = 0;
  let ascIndex = descSortedArr.length - 1;

  for (let i = 0; i < descSortedArr.length; i++) {
    if (i % 2 === 0) {
      resultArr.push(descSortedArr[descIndex]);
      descIndex++;
    } else {
      resultArr.push(descSortedArr[ascIndex]);
      ascIndex--;
    }
  }
