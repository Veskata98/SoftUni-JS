...
// •	Keep in mind that Cupid can have a larger jump length than the size of the
// neighborhood, and if he does jump outside of it, he should start from the first house again (index 0)
// For example, we are given this neighborhood: 6@6@6. Cupid is at the start and jumps with a length of 2.
// He will end up at index 2 and decrease the needed hearts by 2: [6, 6, 4]. Next, he jumps again with a length of
// 2 and goes outside the neighborhood, so he goes back to the first house (index 0) and again decreases the needed hearts there: [4, 6, 4].
// Input
...
function heartDelivery(arr) {
  ...
  for (let el of arr) {
    if (el === "Love!") {
      console.log(`Cupid's last position was ${currentPos}.`);
      let failedPlaces = 0;
      ...
      if (!failedPlaces) {
        ...
        return `Cupid has failed ${failedPlaces} places.`;
      }
    }
    let jumpPos = +el.split(" ").pop();
    currentPos += jumpPos;
    if (currentPos >= houses.length) {
      currentPos = 0;
    }

    if (houses[currentPos] - 2 > 0) {
      houses[currentPos] -= 2;
    } else if (houses[currentPos] - 2 === 0) {
      houses[currentPos] -= 2;
      console.log(`Place ${currentPos} has Valentine's day.`);
    } else {
      console.log(`Place ${currentPos} already had Valentine's day.`);
    }
  }
}
