...
// Check if both indexes are valid and if not, skip the command. If the section
// breaks (health <= 0) the pirate ship sinks, print the following and stop the program:
...
// •	"Status" - prints the count of all sections of the pirate ship that need repair soon, which are all
// sections that are lower than 20% of the maximum health capacity. Print the following:
    ...
    switch (command) {
      case "Fire":
        ...
        if (fireIndex >= 0 && fireIndex < warShip.length) {
          if (warShip[fireIndex] - fireDamage <= 0) {
      ...
      case "Defend":
        ...
        if (
          ...
          for (let i = startIndex; i <= endIndex; i++) {
            if (pirateShip[i] - defendDamage <= 0) {
      ...
      case "Repair":
        let repairIndex = el[0];
        let healing = el[1];
        if (repairIndex >= 0 && repairIndex < pirateShip.length) {
          if (pirateShip[repairIndex] + healing < maxHealth) {
            pirateShip[repairIndex] += healing;
          } else {
