...
// If the ladybug on index 0 needs to fly to its right by the length of 1 (0 right 1) it will attempt to land on index 1 but
// as there is another ladybug there it will continue further to the right by an additional length of 1, landing on index 2.
...
function ladybugs(arr) {
  let size = arr[0];
  let initState = arr[1].split(" ");

  let field = new Array(size);

  for (let i = 0; i < field.length; i++) {
    if (initState.includes(i.toString())) {
      field[i] = 1;
    } else {
      field[i] = 0;
    }
  ...
  for (let i = 2; i < arr.length; i++) {
    let [bugIndex, direction, flyLength] = arr[i].split(" ");

    bugIndex = +bugIndex;
    flyLength = +flyLength;

    let constFly = flyLength;

    if (field[bugIndex] === 0 || bugIndex > size - 1 || bugIndex < 0) {
      continue;
    }
    ...
    switch (direction) {
      case "left":
        ...
        if (bugIndex - flyLength >= 0 && bugIndex - flyLength < size) {
      ...
      case "right":
        ...
        if (bugIndex + flyLength < size && bugIndex + flyLength >= 0) {
          field[bugIndex + flyLength] = 1;
        }
        break;
    }
