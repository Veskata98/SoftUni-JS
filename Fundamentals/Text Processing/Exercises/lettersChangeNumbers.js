...
function lettersChangeNumbers(data) {
  ...
  for (let el of data) {
    ...
    if (firstLetter === firstLetter.toUpperCase()) {
      firstLetterType = "upper";
    } else {
      firstLetterType = "lower";
    }

    if (lastLetter === lastLetter.toUpperCase()) {
      lastLetterType = "upper";
    } else {
      lastLetterType = "lower";
    }

    firstLetter = firstLetter.toLowerCase();
    lastLetter = lastLetter.toLowerCase();

    firstLetter = firstLetter.charCodeAt(firstLetter) - 96;
    lastLetter = lastLetter.charCodeAt(lastLetter) - 96;
    ...
    if (firstLetterType === "upper") {
      number /= firstLetter;
    } else {
      number *= firstLetter;
    }

    if (lastLetterType === "upper") {
      number -= lastLetter;
    } else {
      number += lastLetter;
    }
